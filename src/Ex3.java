...
public class Ex3 {
	...
	String diaEscolhido = input.next();
        ...
        // diaEscolhido = Normalizer.normalize(diaEscolhido, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "").toLowerCase();
        
	int indiceDia = 0; // int indiceDia = -1;

	for (int i = 0; i < diasSemana.length; i++) {
            
            if (diasSemana[i] == (diaEscolhido)) { // if (diasSemana[i].equalsIgnoreCase(diaEscolhido)) {
        ...
        // Validação caso o usuário insira um texto inválido
	...
	int hora = input.nextInt();
        ...
        // Validação para verificar se o horário está dentro do limite
        
	soma = 0;
	for (int i = 0; i < 7; i++) {
            soma += temperaturas[hora][i]; // soma += temperaturas[i][hora];
	}
